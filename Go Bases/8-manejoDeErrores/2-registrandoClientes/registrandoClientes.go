...
desarrolla una función para validar que todos los datos a registrar de un cliente contienen un valor distinto de cero
Esta función debe retornar, al menos, dos valores. Uno de los valores retornados deberá ser de tipo error
...
// Registrar datos de nueves clientes
...
func readEmployeeDetailsInTxtFile() {
	_, err := os.ReadFile("./Go Bases/8-manejoDeErroresEnGo/2-registrandoClientes/customers.txt")
	if err != nil {
...
func validateNonNullFields(c cliente) (client cliente, err error) {
	if c.Legajo == "" || c.NombreCompleto == "" || c.DNI == "" || c.Teléfono == "" || c.Domicilio == "" {
		return cliente{}, errors.New("uno de los campos del cliente es null")
	}
	return c, nil
}

func main() {
	docket1 := generateDocket()
	client1 := cliente{
		Legajo:         docket1,
		NombreCompleto: "Dionys Seidel",
		DNI:            "",
		Teléfono:       "+54 9 3743 82-4206",
		Domicilio:      "96 436",
	}
	defer func() {
		err := recover()
		if err != nil {
			fmt.Println(err)
			_, err2 := validateNonNullFields(client1)
			if err2 != nil {
				fmt.Println(err2)
			}
